...
install:
        ...
        @if [ `pidof smartpireadout` > "0" ];then killall smartpireadout; fi

        @if [ `pidof smartpiserver` > "0" ]; then killall smartpiserver; fi

        @if [ `pidof smartpimodbusserver` > "0" ]; then killall smartpimodbusserver; fi

        @if [ `pidof smartpiftpupload` > "0" ]; then killall smartpiftpupload; fi
#
...
#
        ...
        cp $(BUILDPATH)/config/etc/smartpi /etc/smartpi
